...
def adminLogin():
    ...
    if loginName == 'admin' and loginPassword == 1234:
        ...
        def userDataCustomer():
            userWindowCustomer = Tk()
            userWindowCustomer.geometry("1024x768")
            userWindowCustomer.title("User Window")

            userWindowTreeView = ttk.Treeview(userWindowCustomer, column=('uc1','uc2','uc3','uc4','uc5','uc6'),
                                              show='headings',height=15)
            userWindowTreeView.place(x=350,y=125)

            userWindowTreeViewPersonnel = ttk.Treeview(userWindowCustomer,
                                                       column=('up1','up2','up3','up4','up5','up6','up7'),height=15,
                                                       show='headings')
            userWindowTreeViewPersonnel.place(x=350,y=325)

            userWindowTreeView.column("# 1", anchor=CENTER)
            userWindowTreeView.heading("# 1", text='Name')
            userWindowTreeView.column("# 2", anchor=CENTER)
            userWindowTreeView.heading("# 2", text='Surname')
            userWindowTreeView.column("# 3", anchor=CENTER)
            userWindowTreeView.heading("# 3", text='ID')
            userWindowTreeView.column("# 4", anchor=CENTER)
            userWindowTreeView.heading("# 4", text='E-Mail')
            userWindowTreeView.column("# 5", anchor=CENTER)
            userWindowTreeView.heading("# 5", text='Phone Number')
            userWindowTreeView.column("# 6", anchor=CENTER)
            userWindowTreeView.heading("# 6", text='Birth')

            userWindowTreeViewPersonnel.column("# 1", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 1", text='Name')
            userWindowTreeViewPersonnel.column("# 2", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 2", text='Surname')
            userWindowTreeViewPersonnel.column("# 3", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 3", text='ID')
            userWindowTreeViewPersonnel.column("# 4",anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 4", text='Salary')
            userWindowTreeViewPersonnel.column("# 5", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 5", text='E-Mail')
            userWindowTreeViewPersonnel.column("# 6", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 6", text='Phone Number')
            userWindowTreeViewPersonnel.column("# 7", anchor=CENTER)
            userWindowTreeViewPersonnel.heading("# 7", text='Birth')

            userEntry = ttk.Entry(userWindowCustomer,width=20)
            userEntry.place(x=150,y=125)

            userEntryPersonnel = ttk.Entry(userWindowCustomer, width=20)
            userEntryPersonnel.place(x=150, y=325)

            def addCustomer():
                addCustomerUser = sqlite3.connect("connCustomer.db")
                cursorAddCustomerUser = addCustomerUser.cursor()

                cursorAddCustomerUser.execute("SELECT * FROM customer")
                allCustomer = cursorAddCustomerUser.fetchall()

                cursorAddCustomerUser.execute("SELECT * FROM customer WHERE id=(?)",(userEntry.get(),))
                selectedCustomer = cursorAddCustomerUser.fetchall()
                addCustomerUser.commit()
                for k in range(len(allCustomer)):
                    userWindowTreeView.insert('','end',text='{}'.format(k),values=(selectedCustomer[k]))

                addCustomerUser.commit()
                addCustomerUser.close()

            def addPersonnel():
                addPersonnelUser = sqlite3.connect("connPersonnel.db")
                cursorAddPersonnelUser = addPersonnelUser.cursor()

                cursorAddPersonnelUser.execute("SELECT * FROM personnel")
                allPersonnel = cursorAddPersonnelUser.fetchall()

                cursorAddPersonnelUser.execute("SELECT * FROM personnel WHERE id=(?)",(userEntryPersonnel.get(),))
                selectedPersonnel = cursorAddPersonnelUser.fetchall()
                addPersonnelUser.commit()

                for m in range(len(allPersonnel)):
                    userWindowTreeViewPersonnel.insert('','end',text='{}'.format(m),values=(selectedPersonnel[m]))

                addPersonnelUser.commit()
                addPersonnelUser.close()

            userAdd = ttk.Button(userWindowCustomer,text='Add Customer',command=addCustomer)
            userAdd.place(x=150,y=150)

            userAddPersonnel = ttk.Button(userWindowCustomer,text='Add Personnel',command=addPersonnel)
            userAddPersonnel.place(x=150,y=350)
            userWindowCustomer.mainloop()
        ...
        accountSettingsMenu.add_command(label='User Data', command=userDataCustomer)
        ...
        deleteFromuserWindowTreeView = ttk.Button(root,text='Delete Customer From Treeview',command=deleteuserWindowTreeView)
        deleteFromuserWindowTreeView.place(x=1200,y=550)
